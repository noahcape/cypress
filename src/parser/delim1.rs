...
/// Creates a new `PDelim` parser combinator that parses one or more
/// occurrences of `p` separated by `delim`.
/// See [`crate::parser::delim::pdelim`] to allow zero or more occurances of `p`
///
/// # Arguments
///
...
/// sequences of `p` one or more times separated by `delim`, returning
...
{
    /// Parses one or more occurrences of `p` separated by `delim` from the input.
    ///
